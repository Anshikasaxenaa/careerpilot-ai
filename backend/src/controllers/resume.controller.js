...
const { extractTextFromPDF, cleanText } = require('../services/resume/resume.service');
const { analyzeResume } = require('../services/ai/ai.service');
...
exports.reAnalyzeResume = async (req, res, next) => {
  try {
    ...
    if (!resume.extractedText) {
      return res.status(400).json({ success: false, message: 'No text found in resume to analyze.' });
    }

    const analysis = await analyzeResume(resume.extractedText, req.body.targetRole || resume.targetRole);
    ...
    res.json({ success: true, message: 'Resume re-analyzed.', resume: updated });
  } catch (error) {
    next(error);
  }
};
